...
namespace EnCryptionDecryption
{
	...
	internal class AES
	{
		...
		internal byte[] Encrypt(byte[] datatToEncrypt, byte[] key, byte[] iv)
		{
			...
			using (var _AES = new AesCryptoServiceProvider())
			{
				_AES.Mode = CipherMode.CBC; //CipherMode.CBC: Sets the encryption mode to Cipher Block Chaining (CBC). CBC is a common mode that ensures that identical blocks of plaintext don't produce the same ciphertext, enhancing security.
				_AES.Padding = PaddingMode.PKCS7; //PaddingMode.PKCS7: Specifies the padding scheme to use. Padding is often necessary because block cipher algorithms (like AES) operate on fixed-size blocks of data. PKCS7 is a standard padding scheme.
				_AES.Key = key; //_AES.Key = key;: Sets the encryption key. This key should be a strong, randomly generated key that is the same length as the AES key size you want to use (e.g., 128, 192, or 256 bits).
				_AES.IV = iv; //_AES.IV = iv;: Sets the initialization vector (IV). The IV is a random value that ensures that encrypting the same plaintext with the same key produces different ciphertext each time, further increasing security.
				...
				using (var memorystream = new MemoryStream())
				{
					...
					var cryptoStream = new CryptoStream(memorystream, _AES.CreateEncryptor(), CryptoStreamMode.Write);
		...
		internal byte[] Decrypt(byte[] datatToDecrypt, byte[] key, byte[] iv)
		{
			using (var _AES = new AesCryptoServiceProvider())
			{
				...
				using (var memorystream = new MemoryStream())
				{
					var cryptoStream = new CryptoStream(memorystream, _AES.CreateDecryptor(), CryptoStreamMode.Write);
